...
class Synchronizer {
    ...
    async runForErrorProvider(errorConfig, result) {
    ...
    async syncErrorGroup(errorGroup, errorConfig) {
        var _a;
        ...
        errorGroup.ticket = await this.config.cacheProvider.getObject(errorGroup.clientId, models_1.CacheName.Tickets);
        errorGroup.alert = await this.config.cacheProvider.getObject(errorGroup.clientId, models_1.CacheName.Alerts);
        ...
        await this.config.cacheProvider.setObject(errorGroup.clientId, errorGroup.ticket, models_1.CacheName.Tickets, false);
        if (!errorGroup.alert) {
        ...
        await this.config.cacheProvider.setObject(errorGroup.clientId, errorGroup.alert, models_1.CacheName.Alerts, false);
    }
    createErrorGroup(error, sourceName) {
        ...
        return {
            ...
            clientId,
            mixpanelIds: (_a = error.mixpanelIds) !== null && _a !== void 0 ? _a : [],
            userEmails: (_b = error.userEmails) !== null && _b !== void 0 ? _b : [],
            count: error.count,
    ...
    addToErrorGroups(error, errorGroups, sourceName) {
        ...
        for (let i = 0; i < errorGroups.length; ++i) {
            ...
            if (newErrorGroup.name === existingErrorGroup.name) {
                ...
                existingErrorGroup.mixpanelIds = Array.from(new Set([...existingErrorGroup.mixpanelIds, ...((_a = error.mixpanelIds) !== null && _a !== void 0 ? _a : [])]));
                existingErrorGroup.userEmails = Array.from(new Set([...existingErrorGroup.userEmails, ...((_b = error.userEmails) !== null && _b !== void 0 ? _b : [])]));
                return;
            }
        }
    ...
    doesTicketNeedReopening(existingTicket) {
        ...
        if (!existingTicket.resolutionDate) {
            return true;
        }
        const currentDate = new Date();
    ...
    doesTicketNeedUpdate(existingTicket, freshTicketContent) {
        console.log('existingTicket: ', existingTicket);
        console.log('freshTicketContent: ', freshTicketContent);
        return !providers_1.JiraTicketProvider.sameTicketContent(existingTicket, freshTicketContent);
    }

...
const makeReportUrl = (message, mixpanelIds) => {
    ...
    const settings = jsurl_1.default.parse(`~(~(resourceType~'event~propertyName~'message~propertyObjectKey~null~propertyDefaultType~'string~propertyType~'string~filterOperator~'contains~filterValue~(~'placeholder)~limitValues~false~defaultEmpty~false~activeValue~(~'placeholder))~(resourceType~'event~propertyName~'!distinct_id~propertyObjectKey~null~propertyDefaultType~'string~propertyType~'string~filterOperator~'equals~filterValue~(~'placeholder1~'placeholder2~'placeholder3)~limitValues~false~defaultEmpty~false~activeValue~(~'placeholder1~'placeholder2~'placeholder3)))`);
    settings[0].filterValue = [message];
    settings[0].activeValue = [message];
    settings[1].filterValue = mixpanelIds;
    settings[1].activeValue = mixpanelIds;
    searchParams.set('filters', jsurl_1.default.stringify(settings));
